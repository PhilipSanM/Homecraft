...
class VisualizerExecutor(ft.Container):
    def __init__(self, page):
        ...
        self.text_message = ft.Text("Visualizando fondo...", size=32, color="white")
    ...
    async def run_visualizer(self):
        visualizer_commands = [
            ...
            'docker exec -it nerfstudio_container bash -c "ns-train splatfacto --data ./nerfstudio/objects/background --steps_per_save 100"',
            # 
        ]
...
def load_background_view(page, appbar):
    page.title = "HomeCraft - Cargando visualizador"
    page.update()
    ...
    visualizer_executor = VisualizerExecutor(page)
